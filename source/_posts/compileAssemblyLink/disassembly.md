...
```
switch 不管case分支多少(测试要5个分支及以上)，采用数组索引的方式，直接跳转，相比if语句不仅占用空间小，而且耗时上是O(1)的算法。
如果case 不连续怎么办，自己动手反汇编看下代码吧
...
```
没有亲自动手反汇编之前，你可能以为通过栈传递参数的。
但x86-64通过寄存器传递参数(参数个数小于等于6，超过6个的通过栈传递)。
你可能想到这不是fastcall嘛，但其实x86-64中只有一种调用约定：调用方清理堆栈。
一般记住：e[r]di 是第一个参数，e[r]si 是第二个参数，剩下就百度把。
...
```
\<mmap@plt\>是什么，可以看我之前写的[《动态链接》](https://liji53.github.io/2021/05/26/compileAssemblyLink/dynamicLd/)
...
00000000000f8d10 <mmap>:
   ...
   ...
```
系统调用号放在eax中，其他参数放在其他通用寄存器中(可以百度)，返回值通过rax。
它的流程图：
...
##### 3.初始化列表
看到现在，我们应该对反汇编已经有点感觉了，因此找了初始化列表做例子(够简单)，通过阅读汇编，看看能不能猜出对应的数据结构与实现算法
```c++
template<class _E>
...
看完反汇编代码，自己去linux上找源码，注意：vs的实现跟gnu的实现并不一致。

### 参考资料
书籍： 《老码识途 从机器码到框架的系统观逆向修炼之路》
       《C++反汇编与逆向分析技术揭秘》
       《汇编语言》
       《程序员的自我修养》
