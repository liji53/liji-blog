...
---
# 文件系统
文件系统是OS用来组织和分配存储设备的方法，现在linux系统用的基本都是Ext4文件系统，在window上则基本都是NTFS文件系统，接下来我们将学习它们的数据结构与实现原理。
...
2. 寻道：图中seek的动作，寻道时间一般在几毫秒到十几毫秒
3. 旋转：图中Rotates的动作，旋转一圈的时间一般也要几毫秒(如10000RPM = 6 R/ms)
...
但为什么要设计成12个direct block以及3个一级、二级、三级间接block呢？（细品这些数据结构）
1. 设计成多级间接block是为了支持大容量的文件：(12 + 1024 + 1024\^2 + 1024\^3) × 4 KB
2. 设计成12个direct block，是因为大多数文件的大小只有2k，这样就可以直接定位到block，而不用去data block间接查找，提高效率
...
　　};
```
目录数据会保存此目录下的所有文件名和目录名，以及文件的入口地址。至于其他的信息则存放在inode节点中
...
2. 文件下一个数据块的簇号，因此不需要类似inode的block字段

但从上面的布局中，我们没有找到用于描述文件信息的元数据，其实这些元信息都存放在目录项中，而目录项的数据又存放在数据块中，因此这里看不到。
...
这里我们仅讨论对元数据的修复，因此像数据块丢失、不一致的问题可以在数据库事务中再讨论。
